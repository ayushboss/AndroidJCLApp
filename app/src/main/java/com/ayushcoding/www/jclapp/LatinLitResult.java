...
public class LatinLitResult extends AppCompatActivity {
    TextView score;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_latin_lit_result);
        score = (TextView) findViewById(R.id.scoreLL);
        Bundle b = getIntent().getExtras();
        final int scoreRHTHING= b.getInt("scoreRl");
        Context context = this;

        score.setText(""+ scoreRHTHING+ "");
        final ScoresDB scoresDB = new ScoresDB(LatinLitResult.this);
...
//                        ScoresDB scoresDB = new ScoresDB(GrammarFinished.this);
                        scoresDB.addScore("latinLit", scoreRHTHING, 10);
                        break;

                    case DialogInterface.BUTTON_NEGATIVE:
                        //No button clicked
//                        scoresDB = new ScoresDB(GrammarFinished.this);
                        scoresDB.ScoreUpdate("latinLit", scoreRHTHING);
                        break;
                }
            }
        };
